...
def get_target_hosts():
    ...
    if dynamic_args['host']:
        if dynamic_args['host'] in hosts.keys():
            my_target_hosts[dynamic_args['host']] = 1
    elif dynamic_args['tag']:
...
def print_datapoints(datapoints):
    ...
    #        mydict = datapoint['tags']
    #        tags=''
    #        for k,v in datapoint['tags'].items()[:-1]:
    #            if (type(v) is int) or (type(v) is float):
    #                tags=tags+k+"="+v+","
    #            elif type(v) is str:
    #                tags=tags+k+"="+"'"+re.escape(str(v))+'"'+","
    #        for k,v in datapoint['tags'].items()[-1]:
    #            if (type(v) is int) or (type(v) is float):
    #                tags=tags+k+"="+v
    #            elif type(v) is str:
...
#
    #        fields=''
    #        for k,v in datapoint['fields'].items()[:-1]:
    #            if (type(v) is int) or (type(v) is float):
    #                fields=fields+k+"="+v+","
    #            elif type(v) is str:
    #                fields=fields+k+"="+"'"+re.escape(str(v))+'"'+","
    #        for k,v in datapoint['fields'].items()[-1]:
    #            if (type(v) is int) or (type(v) is float):
    #                fields=fields+k+"="+v
    #            elif type(v) is str:
    #                fields=fields+k+"="+"'"+re.escape(str(v))+'"'
#
    #        #tags=','.join("%s=%s" % (k,v) for k,v in datapoint['tags'].items())
    #        #fields=','.join("%s=%s" % (k,'"'+re.escape(str(v))+'"') for k,v in datapoint['fields'].items())
    #        print "%s,%s %s" % (datapoint['measurement'],tags,fields)
    #elif isinstance(datapoints, dict): # We asume it's a dict instead of a list
    if isinstance(datapoints, dict): # We asume it's a dict instead of a list     
        tags=','.join("%s=%s" % (k,v) for k,v in datapoints['tags'].items())
        #fields=','.join("%s=%s" % (k,'"'+re.escape(str(v))+'"') for k,v in datapoints['fields'].items())
        fields = None
        for k,v in datapoints['fields'].iteritems():
            if fields is None:
                if (type(v) is int) or (type(v) is float):
                    fields=str(k)+"="+str(v)
                elif type(v) is str:
                    fields=str(k)+"="+'"'+re.escape(str(v))+'"'
            else:
                if (type(v) is int) or (type(v) is float):
                    fields=fields+','+str(k)+"="+str(v)
                elif type(v) is str:
                    fields=fields+','+str(k)+"="+'"'+re.escape(str(v))+'"'+","
        
        print "%s,%s %s" % (datapoints['measurement'],tags,fields)
    else:
        logger.debug('[%s]: unknown type variable type: %s', type(datapoints))
...
def get_metadata_and_add_datapoint(datapoints,**kwargs):
    ...
    kpi["tags"] = kpi_tags
    # FUTURE IMPROVEMENT: Let's print the kpi here, instead of append to a big json
    #datapoints.append(copy.deepcopy(kpi))
    print_datapoints(datapoints=copy.deepcopy(kpi))
        ...
        if "non_junos_devices" in hosts[host].split():
            ...
            if connected:
                ...
                if result:
                    ...
                    #NOTE in this scenario value_tmp is being set to 0 because the same info is being used in product-model, 
                    # We'll add a dummy kpi in oder to have at least one fixed kpi with version/platform data.
                    #get_metadata_and_add_datapoint(datapoints=datapoints,match=match,value_tmp=value_tmp,latest_datapoints=latest_datapoints,host=host,kpi_tags=kpi_tags)
                    get_metadata_and_add_datapoint(datapoints=datapoints,match=match,value_tmp=0,host=host,kpi_tags=kpi_tags)
...
full_parser.add_argument("--tag", nargs='+', help="Collect data from hosts that matches the tag (not compatible with --host option)")
full_parser.add_argument("--host", help="Target host (not compatible with --tag option)")
full_parser.add_argument("--start_delay", help="Start delay in seconds")
...
default_variables_yaml_file = BASE_DIR_INPUT + "variables/input-netconf-variables.yaml"
default_variables = {}
...
start_delay = 0
###  Known and fixed arguments
if dynamic_args['start_delay']:
    start_delay = dynamic_args['start_delay']
...
#if dynamic_args['host']:
#    logger.error('Host option specified, ignoring tags filter (if any)')
#    sys.exit(0)      
...
try:
    with open(hosts_yaml_file) as f:
        hosts_tmp = yaml.load(f)
    hosts=hosts_tmp['hosts']
except Exception, e:
